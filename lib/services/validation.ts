 ...
 * VERSION SIMPLIFIÉE - Accepte les query params null/undefined
 */
...
/**
 * Valide et convertit une chaîne en ObjectId MongoDB
 */
...
export const paginationSchema = z.object({
  page: z.union([z.string(), z.number(), z.null(), z.undefined()]).optional().transform((val) => {
    if (val === null || val === undefined || val === '') return 1
    const num = typeof val === 'string' ? parseInt(val, 10) : val
    return isNaN(num) || num < 1 ? 1 : num
  }),
  limit: z.union([z.string(), z.number(), z.null(), z.undefined()]).optional().transform((val) => {
...
export const creerPersonnelSchema = z.object({
  ...
  prenom: z.string().min(1).max(100),
  role: z.enum([
    'directeur', 'enseignant', 'censeur', 'surveillant', 'informaticien',
    'secretaire', 'gestionnaire', 'infirmier', 'bibliothecaire', 'autre'
  ]),
